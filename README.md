...
|  **UAC Desteği** | Dahili manifest yapısı sayesinde kısıtlı erişim hatalarını önlemek için otomatik olarak Admin yetkisiyle çalışır. |

## ⚙️ Kurulum ve Derleme

Projeyi derlemek için sisteminizde **MinGW (GCC/g++)** kurulu olmalıdır.
*<small>Alternatif olarak Release kısmından buildlenen exeyi kullanabilirsiniz</small>*
...
> 💡 **Not:** Derleme tamamlandığında dizinde otomatik olarak yönetici kalkanına (UAC Admin) sahip `PersistenceScanner.exe` dosyanız oluşacaktır.

## 💻 Operasyon Akışı

Aracı (`PersistenceScanner.exe`) çalıştırdığınızda şu adımlar sırasıyla gerçekleşir:
...
3. **Filtreleme & Ayıklama:** Kullanıcı tercihine göre güvenilir sistem dosyaları ve sahte girdiler fazlalığı azaltmak için filtrelenir.
